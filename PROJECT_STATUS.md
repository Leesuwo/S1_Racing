...
- 제품: S1 Racing
- 작업 기준일: 2026-07-27
- 저장소 문서상 완료 상태: Milestone 2B → 2C → 2D → 3A
- 다음 기능 마일스톤: Milestone 3B — 타이어 열·마모·공기압 모델
- 현재 오케스트레이션 상태: 트랙 리밋·벽·연석·차량 접촉 경계와 브라우저 표시 구현·검증 완료
...
- M3A의 차량 접촉은 RaceSession의 2D 원형 근사이며, Rapier world에서 여러 차량의 차체 형상·회전까지 함께 적분하지 않는다.
- M3A의 트랙 리밋 패널티·벽·연석 수치는 initial_assumption이고, 손상·추월·방어·충돌 회피는 구현하지 않는다.
- M2B의 `retired`는 비정상 수치 방어 상태이며 접촉 손상 상태가 아니다.
- 퀄리파잉 랩 기록은 규칙·단계 전이를 검증하는 결정적 프로토타입 시나리오이며, 실제 차량 랩 완주 기반 타이밍·섹터 델타는 후속 범위다.
- 타이어 전략은 시작 컴파운드와 최소 1회 피트 컴파운드의 유효성만 확인하며, 열·마모·공기압·실제 피트 물리는 M3B 이후다.
...
2026-07-27 M3A 결과: 타입 검사·단위 테스트 85건·아키텍처 검증·프로덕션 빌드·브라우저 E2E 15건 통과.
