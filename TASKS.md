...
- 상태: M2A-0 설정 전달·공유 물리 경계·AI 텔레메트리 검증 완료 (2026-07-27)
- 완료된 범위: 교육 결과 JSON 저장, 단일 AI 주행 모드, 동일 `VehicleControlInput`·`VehicleSimulation`·Rapier 힘 적용 순서, AI 목표 속도·레이싱 라인·제동 단위 테스트·브라우저 E2E
- 남은 범위: 다차량 세션과 기본 순위는 M2B에서 진행
...
- 선행 조건: M2A 검증 게이트 통과
- 범위: 여러 AI 차량 스폰·그리드·충돌 없는 기본 순위·세션 리셋, 고정 스텝 비용 측정
- 제외: 퀄리파잉, 피트 전략, 추월·방어·실제 충돌 모델
- 필수 검증: 차량 수별 고정 스텝 시간·순위 결정성·세션 리셋 E2E, `npm run verify`
