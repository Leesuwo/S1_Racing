...
2. 이 문서와 `roles/*.md` — 역할별 책임과 금지 범위
3. `TASKS.md` — 현재 작업 큐와 배치 상태
4. `PROJECT_STATUS.md` — 완료 기준, 알려진 위험, 현재 상태
5. `docs/ARCHITECTURE.md`, `docs/DECISIONS.md`, 분야별 명세 — 설계 계약
...
| 역할 | 모델 | Reasoning | 사용 범위 |
|---|---|---|---|
| Lead | `gpt-5.6-terra` | `high` | 요구사항 분해, 통합, 충돌 해결, 최종 판정 |
| Writer/Worker | `gpt-5.6-luna` | `medium` | 문서, 테스트, 단순 UI, 명확히 bounded된 구현 |
| Explorer | `gpt-5.6-luna` | `medium` | 읽기 전용 탐색·요약·로그 분석 |
| Physics | `gpt-5.6-terra` | `high` | 타이어·구동계·고정 스텝·수치 경계 |
| QA | `gpt-5.6-terra` | `high` | 회귀, 결정성, 경계, 검증 증거 리뷰 |
...
사용자 요청
  ↓
Lead Agent: 분해·소유권·합격 기준 확정
  ↓
전문 구현/분석 Agent 1~3개
  ↓
QA Agent: 읽기 전용 검토와 재현 절차
  ↓
Lead Agent: 피드백 반영·통합·전체 검증
  ↓
...
현재 저장소는 `TASKS.md`의 M1F를 다음 기능 후보로 둔다. M1F에서는 Physics Agent가 기존 물리 계층을 불필요하게 수정하지 않도록 하고, 입력·트랙 경계와 QA를 우선 배정한다. AI·다차량·레이스 역할은 M1F 합격 이후에만 활성화한다.
