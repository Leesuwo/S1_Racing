...
## 6. 2026-07-29 렌더링 동역학 적용 기록

이번 구현은 물리 위치·속도 소유권을 변경하지 않고, `VehicleRenderSnapshot`에 네 바퀴의 `wheelSpinRad`를 추가해 외관과 물리 상태의 시간축을 맞춘다.

| 항목 | 구현 기준 | 근거·한계 |
|---|---|---|
| 앞바퀴 조향 | 부모 그룹 Y축 회전 | `steeringAngleRad`와 차체 yaw를 분리한다. |
| 네 바퀴 구름 | 자식 그룹 X축 회전 | Rapier 각속도를 우선하고, 순수 AI 경로는 속도·휠 반지름 fallback을 사용한다. |
| 회전 부호 | 물리 전방 -Z에 맞춘 반대 부호 | Three.js 시각축과 S1 물리축의 좌표 변환 규칙이다. |
| 콕핏 | 림·미러·헬멧·바이저·팔·휠 | 외부 드라이버·팀 자산 없이 2012년형 오픈 콕핏 시대감만 재구성한다. |
| 공력 형상 | 프런트 윙 스트레이크·리어 윙 엔드플레이트 추가 | 형상 판독용이며 CFD 성능을 주장하지 않는다. |

검증은 `VehicleSimulation.test.ts`의 네 바퀴 회전·역방향 부호 회귀와 Driving·Training·Race Weekend 렌더 연결을 사용한다. 실제 타이어 슬립이나 공력 맵은 화면용 회전량과 별개의 후속 동역학 마일스톤이다.
